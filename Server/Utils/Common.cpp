...
void printf_info(FILE *stream, const char *format, ...)
{
    ...
    printf_real(stream, str, "[INFO]");
}

/**
 * @description: Print warning
 * */
void printf_warn(FILE *stream, const char *format, ...)
{
    va_list va;
	va_start(va, format);
	int n = vsnprintf(print_buf, PRINT_BUF_SIZE, format, va);
    ...
    printf_real(stream, str, "[WARN]");
}

/**
 * @description: Print error
 * */
void printf_err(FILE *stream, const char *format, ...)
{
    va_list va;
	va_start(va, format);
	int n = vsnprintf(print_buf, PRINT_BUF_SIZE, format, va);
 ...
 * @return: print status
 * */
void printf_real(FILE *stream, std::string info, const char *info_tag)
{
	// Print timestamp
	time_t ts;
